...
export function ToolInvocation({ toolInvocations, defaultOpen = false }: ToolInvocationProps) {
  const [isExpanded, setIsExpanded] = useState(defaultOpen);

  const toolInvocationsData = Array.isArray(toolInvocations) ? toolInvocations : [toolInvocations];
      ...
      <div className="flex flex-col gap-0 overflow-hidden rounded-md border border-border">
        ...
        >
          <div className="flex flex-1 flex-row items-center gap-2 text-left text-base">
            <PiNut className="size-4 text-muted-foreground" />
                  ...
                  {uniqueToolIds.map((toolId) => {
                    ...
                    return (
                      <div key={toolId} className="pb-2 last:border-0 last:pb-0">
                        <SingleToolView toolInvocations={toolInvocationsForId} />
                      </div>
                    );
                  })}
                </div>
...
function SingleToolView({ toolInvocations, defaultOpen = false, className }: SingleToolViewProps) {
  ...
  const toolsToDisplay = Object.values(groupedTools)
    .map((group) => {
      const resultTool = group.find((item) => item.toolInvocation.state === "result");
      const callTool = group.find((item) => item.toolInvocation.state === "call");
      const partialCallTool = group.find((item) => item.toolInvocation.state === "partial-call");
  ...
  if (toolsToDisplay.length === 1) {
    const toolData = toolsToDisplay[0];
      ...
      if (
        ...
        return (
          <div className="space-y-3">
            {parsedResult.map(
              (item: { url: string; title: string; snippet?: string }, index: number) => (
                <div key={index} className="border-border border-b pb-3 last:border-0 last:pb-0">
                  <a
                    href={item.url}
                    target="_blank"
                  ...
                  >
                    {item.title}
                    <PiLink className="h-3 w-3 opacity-70 transition-opacity group-hover:opacity-100" />
                  </a>
                  <div className="mt-1 font-mono text-muted-foreground text-xs">{item.url}</div>
                  {item.snippet && <div className="mt-1 line-clamp-2 text-sm">{item.snippet}</div>}
                </div>
              ),
            )}
          </div>
          ...
          {htmlUrl && (
            <div className="mb-2">
              ...
              >
                ...
                <PiLink className="h-3 w-3 opacity-70" />
              </a>
            </div>
          )}
          <div className="font-mono text-xs">
            ...
            <div className="space-y-3 px-3 pt-3 pb-3">
              ...
              {args && Object.keys(args).length > 0 && (
                <div>
                  ...
                  <div className="rounded border bg-background p-2 text-sm">{formattedArgs}</div>
                </div>
              )}
              ...
              <div className="flex items-center justify-between text-muted-foreground text-xs">
                <div className="flex items-center">
                  <PiCode className="mr-1 inline size-3" />
